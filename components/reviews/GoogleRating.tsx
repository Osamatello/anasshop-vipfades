...
export default function GoogleRating({ compact = false, reviewBlock = false }: { compact?: boolean; reviewBlock?: boolean }) {
  ...
  const starSize = reviewBlock ? 'h-3.5 w-3.5' : compact ? 'h-3 w-3 sm:h-5 sm:w-5' : 'h-5 w-5';

  return (
    <div className={`inline-flex items-center text-brand-cream ${compact ? 'min-h-8 gap-2 text-left sm:min-h-12 sm:gap-4' : 'min-h-12 flex-wrap justify-center gap-4'}`}>
      {(
        <a
          href={GOOGLE_REVIEWS_LINK}
          target="_blank"
          rel="noopener noreferrer"
          className={`inline-flex items-center rounded focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-brand-cream ${compact ? 'gap-1.5 sm:gap-3' : 'gap-3'}`}
          aria-label={`${stats.rating.toFixed(1)} von 5 Sternen bei ${stats.totalReviewCount} Google-Bewertungen – auf Google ansehen`}
        >
          <span className={`${ratingFont.className} leading-none tabular-nums ${compact ? 'order-last text-[26px] sm:text-5xl' : 'text-3xl'}`}>{stats.rating.toFixed(1)}</span>
          <span className={compact ? 'text-right' : 'text-left'}>
            <span aria-hidden="true" className="flex gap-px sm:gap-0.5">
            ...
            <span className={`block font-sans font-bold uppercase tracking-[0.1em] tabular-nums ${compact ? 'mt-0.5 text-[8px] sm:mt-1.5 sm:text-[11px] sm:tracking-[0.12em]' : 'mt-1.5 text-[9px] tracking-[0.12em]'}`}>
